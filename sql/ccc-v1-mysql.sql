...
CREATE TABLE jobs (
    ...
    /* The last time we sent an ALERT_ERRORLIMIT alert for this job. That alert
     * type is special, since it will keep triggering forever without
     * administrator action, so that is why that one alone gets a column in
     * this table. A NULL means we haven't sent such an alert for this job yet.
     */
    errorlimit_mtime DATETIME,
